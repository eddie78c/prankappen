export const de = {
  // Welcome & Auth
  welcome: "Willkommen bei",
  bankName: "PRÄMIUMBANK",
  enterPin: "PIN eingeben",
  wrongPin: "Falsche PIN. Versuchen Sie es erneut.",
  
  // Navigation
  home: "Start",
  history: "Verlauf",
  cards: "Karten",
  profile: "Profil",
  settings: "Einstellungen",
  account: "Konto",
  helpAndSupport: "Hilfe & Support",
  logOut: "Abmelden",
  more: "Mehr",

  // Dashboard
  dashboard: "Übersicht",
  overview: "Übersicht, schauen Sie sich Ihr Guthaben an",
  totalBalance: "Gesamtguthaben",
  availableBalance: "Verfügbares Guthaben",
  monthlyIncome: "Einkommen diesen Monat",
  todaySpent: "Heute ausgegeben",
  recentTransactions: "Letzte Transaktionen",
  seeAll: "Alle anzeigen",
  
  // Cards
  yourCards: "Ihre Karten",
  physicalCard: "Physische Karte",
  virtualCard: "Virtuelle Karte",
  cardSettings: "Karteneinstellungen",
  contactlessPayment: "Kontaktlose Zahlung",
  onlinePayment: "Online-Zahlung",
  atmWithdraws: "Geldautomaten-Abhebungen",
  physicalCards: "physische Karte",
  virtualCards: "virtuelle Karte",
  
  // Transactions
  today: "HEUTE",
  yesterday: "GESTERN",
  thisWeek: "DIESE WOCHE",
  
  // Actions
  send: "Senden",
  request: "Anfordern",
  loan: "Darlehen",
  topup: "Aufladen",
  add: "Hinzufügen",
  
  // Categories
  grocery: "Lebensmittel",
  transport: "Transport",
  salary: "Gehalt",
  coffee: "Kaffeehaus",
  
  // Settings
  darkMode: "Dunkler Modus",
  language: "Sprache",
  notifications: "Benachrichtigungen",
  security: "Sicherheit",

  // Mock data translations
  mainGroceryStore: "REWE Berlin",
  slCardTopUp: "SL Card Aufladung",
  monthlySalary: "Monatsgehalt",
  espressoHouse: "Espresso House",
  housing: "Wohnen",
  food: "Essen",
  entertainment: "Unterhaltung",
  budgetOverview: "Budgetübersicht",

  // More screen translations
  selectLanguage: "Sprache auswählen",
  selectCurrency: "Währung auswählen",
  profileConfiguration: "Profilkonfiguration",
  prankConfiguration: "Prank-Konfiguration",
  profileName: "Profilname",
  location: "Standort",
  enterProfileName: "Profilnamen eingeben",
  enterLocation: "Standort eingeben",
  enterBalance: "Saldo eingeben",
  receiverName: "Empfängername",
  defaultAmount: "Standardbetrag",
  enterReceiverName: "Empfängernamen eingeben",
  enterAmount: "Betrag eingeben",
  receiverPhoto: "Empfängerfoto",
  requestSound: "Anfrageton",
  typesOfLaughter: "Arten von Lachen",
  soundSelected: "Ton ausgewählt",
  aPaySound: "A-Pay-Ton wird bei Anfrage abgespielt",
  saveProfile: "Profil speichern",
  saveSettings: "Einstellungen speichern",
  close: "Schließen",
  version: "Version 1.0.0",
  copyright: "© 2025 Premium Bank. Alle Rechte vorbehalten.",
  madeWith: "Mit ❤️ für sicheres Banking gemacht",
  permissionNeeded: "Berechtigung erforderlich",
  cameraPermission: "Wir benötigen Kameraroll-Berechtigung zum Foto-Auswählen.",
  error: "Fehler",
  failedToPickSound: "Fehler beim Auswählen des benutzerdefinierten Tons",
  settingsSaved: "Einstellungen gespeichert",
  prankSettingsUpdated: "Prank-Einstellungen wurden aktualisiert!",
  profileUpdated: "Profil-Einstellungen wurden aktualisiert!",
  helpCenter: "Hilfe-Center",
  about: "Über",
  configurePrank: "Prank konfigurieren",
  profileSettings: "Profil-Einstellungen",
  appearance: "Aussehen",
  prankSettings: "Prank-Einstellungen",
  services: "Dienstleistungen",
  support: "Unterstützung",

  // Modal translations
  sendMoney: "Geld senden",
  phoneNumber: "Telefonnummer",
  enterPhoneNumber: "Telefonnummer eingeben",
  amount: "Betrag",
  cancel: "Abbrechen",
  insufficientBalance: "Unzureichendes Guthaben",
  cannotSendMore: "Sie können nicht mehr Geld senden als Ihr aktuelles Guthaben",
  pleaseEnterPhone: "Bitte geben Sie eine Telefonnummer ein",
  pleaseEnterValidAmount: "Bitte geben Sie einen gültigen Betrag ein",
  moneyReceived: "Geld erhalten!",
  moneySent: "Geld gesendet!",

  // Notification popup
  notification: "Benachrichtigung",
  prankMessage: "Dies ist eine Scherz-Banking-App nur für Unterhaltungszwecke. Es werden keine echten Transaktionen durchgeführt.",
  gotIt: "Verstanden",

  // Cards
  showCardDetails: "Kartendetails anzeigen",
  hideCardDetails: "Kartendetails ausblenden",
  addNewCard: "Neue Karte hinzufügen",
  cardHolder: "KARTENINHABER",
  expires: "ABLÄUFT",
  cvv: "CVV",

  // Settings
  currency: "Währung",

  // Rating
  rateUs: "Bewerte uns",
  rateAppQuestion: "Wie würden Sie unsere App bewerten?",
  writeComment: "Schreiben Sie Ihren Kommentar (optional)",
  submitRating: "Bewertung senden",
  thankYou: "Danke!",
  thankYouFeedback: "Danke für Ihr Feedback!",

  // Security
  setPinCode: "Stellen Sie Ihren PIN-Code für die App-Sicherheit ein:",
  enter4DigitPin: "4-stelligen PIN eingeben",
  confirm4DigitPin: "4-stelligen PIN bestätigen",
  savePin: "PIN speichern",
  pinSet: "PIN gesetzt",
  pinDisabled: "PIN deaktiviert",
  pinMismatch: "PIN-Fehler",
  invalidPin: "Ungültiger PIN",
  enter4DigitPinOrEmpty: "Geben Sie einen 4-stelligen PIN ein oder lassen Sie ihn leer, um zu deaktivieren.",
  pinsDoNotMatch: "PINs stimmen nicht überein. Versuchen Sie es erneut.",

  // History
  all: "Alle",
  income: "Einkommen",
  expenses: "Ausgaben",

  // Sound
  selectSound: "Ton auswählen",

  // Country-specific chains
  chain1: "REWE Berlin",
  chain2: "Starbucks Berlin",
  chain3: "Lidl Berlin",
  chain4: "Aldi Berlin",
  chain5: "Edeka Berlin",
  chain6: "Kaufland Berlin",
  chain7: "dm Berlin",
  chain8: "Rossmann Berlin",
  chain9: "Saturn Berlin",
  chain10: "Tchibo Berlin",

  // Affiliate Program
  affiliateProgram: "Affiliate-Programm",
  totalEarned: "Insgesamt verdient",
  earnPerReferral: "Verdienen Sie 1 $ pro Empfehlung, wenn jemand die App kauft",
  totalReferrals: "Gesamt-Empfehlungen",
  pendingPayouts: "Ausstehende Auszahlungen",
  referralLink: "Empfehlungslink",
  requestPayout: "Auszahlung anfordern",
  minimumPayout: "Mindestauszahlung: 50 $",
  paypalEmail: "PayPal-E-Mail",
  enterPaypalEmail: "PayPal-E-Mail eingeben",
  payoutAmount: "Auszahlungsbetrag",
  enterPayoutAmount: "Betrag eingeben",
  payoutRequested: "Auszahlung erfolgreich angefordert",
  invalidPaypalEmail: "Bitte geben Sie Ihre PayPal-E-Mail-Adresse ein",
  invalidPayoutAmount: "Bitte geben Sie einen gültigen Auszahlungsbetrag ein",
  minimumPayoutAmount: "Der Mindestauszahlungsbetrag beträgt 50 $. Sie haben derzeit 47,00 $ verfügbar.",
  insufficientEarnings: "Sie können nicht mehr anfordern als Ihre verfügbaren Einnahmen. Sie haben 47,00 $ verfügbar, aber $[amount] angefordert.",
  copied: "Kopiert!"
};